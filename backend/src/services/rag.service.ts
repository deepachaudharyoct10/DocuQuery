...
export async function askQuestion(question: string, conversationId?: string): Promise<AskQuestionResult> {
  ...
  // Fire-and-forget: don't block the chat response on contradiction judging.
  detectContradictions(retrievedChunks, question).catch((err) => {
    logger.error("Contradiction detection failed", { err: err instanceof Error ? err.message : err });
  });
